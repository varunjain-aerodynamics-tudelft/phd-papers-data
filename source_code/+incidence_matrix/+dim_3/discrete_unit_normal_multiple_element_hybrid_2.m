function [local_NK] = discrete_unit_normal_multiple_element_hybrid_2(p, Kx, Ky, Kz, globl_dof_rght, globl_dof_left, globl_dof_topp, globl_dof_bttm, globl_dof_frnt, globl_dof_back)
...
gather_local_dof = gather_matrix.GM_total_local_dof_v3(ttl_nr_el, nr_surfc)';
...
ttl_loc_dof2 = ttl_nr_el * nr_surfc ;
...
for j = 1:Ky
    for k = 1:Kz
        ...
        boundary_face_rght(:,local_eleid) = gather_local_dof(globl_dof_rght,globl_eleid); %% gather local dof will change as required for continuous elements 
    end
end
...
for j = 1:Ky
    for k = 1:Kz
        ...
        boundary_face_left(:,local_eleid) = gather_local_dof(globl_dof_left,globl_eleid);
    end
end
...
for i = 1:Kx
    for k = 1:Kz
        ...
        boundary_face_topp(:,local_eleid) = gather_local_dof(globl_dof_topp,globl_eleid);
    end
end
...
for i = 1:Kx
    for k = 1:Kz
        ...
        boundary_face_bttm(:,local_eleid) = gather_local_dof(globl_dof_bttm,globl_eleid);
    end
end
...
for i = 1:Kx
    for j = 1:Ky
        ...
        boundary_face_frnt(:,local_eleid) = gather_local_dof(globl_dof_frnt,globl_eleid);
    end
end
...
for i = 1:Kx
    for j = 1:Ky
        ...
        boundary_face_back(:,local_eleid) = gather_local_dof(globl_dof_back,globl_eleid);
    end
end
...
local_NK = sparse(1:nr_boundary_lambda,local_bndry_dof_NK,NNK,nr_boundary_lambda,ttl_loc_dof2);
