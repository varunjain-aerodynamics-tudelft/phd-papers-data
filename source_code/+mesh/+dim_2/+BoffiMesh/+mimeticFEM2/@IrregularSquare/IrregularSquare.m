classdef IrregularSquare %< mimeticFEM2.Mesh
    ...
    methods
        ...
        %         function nodesOfElements = computeNodesOfElements(obj, deformation)
        %
        ...
        %                 nodesCoordinatesY(iDeformed, jDeformed) = nodesCoordinatesY(iDeformed, jDeformed) + deformation * 2.0/(obj.n(2));
        ...
        function nodesOfElements = computeNodesOfElements(obj, deformation)
            ...
            if deformation > 0
                ...
                % First change the points that raise in the y-direction
                % (even points in x and even in y)
                iDeformed = 2:2:obj.n(2);  % index for y points
                jDeformed = 2:2:(obj.n(1)+1);  % index for x points
                nodesCoordinatesY(iDeformed, jDeformed) = nodesCoordinatesY(iDeformed, jDeformed) + deformation * 2.0/(obj.n(2));
                
                % Then change the points that lower in the y-direction
                % (odd points in x and even in y)
                jDeformed = 1:2:(obj.n(1) + 1);
                nodesCoordinatesY(iDeformed, jDeformed) = nodesCoordinatesY(iDeformed, jDeformed) - deformation * 2.0/(obj.n(2));
            end
